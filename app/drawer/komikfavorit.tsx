...
export default function KomikFavorit() {
  ...
  const searchComics = async (query = "") => {
    try {
      ...
      const options = {
        ...
        body: `cari=${query}&user_id=${userId}`,
      };

      const response = await fetch(
        "https://ubaya.xyz/react/160421050/uas/komikfavorit.php",
        options
      );
      ...
      if (json.result === "success") {
        ...
        alert("Failed to fetch comics: " + (json.message || "Unknown error"));
      }
    } catch (error) {
  ...
  const searchComicsCategory = async (query = "") => {
    try {
      ...
      const options = {
        ...
        body: `kategori=${query}&user_id=${userId}`,
      };
      ...
      console.error("Error searching comics by category:", error);
    }
  };
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      {results.length === 0 ? (
      <Text style={styles.emptyText}>Belum ada komik favorit</Text>
      ) : (
      <FlatList
        data={results}
